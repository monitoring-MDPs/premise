...
if [ $# -lt 2 ]; then
    echo "Usage: $0 <stats_path> <model_path> <log_path>"
    exit 1
fi
...
for i in $(seq 1 $n); do
    ...
    # python -m premise.interval.run_conformal_prediction 12h $stats_path -mc refuelB-12-50 --dump-model $model_path --dump-stats $stats_path --no-target --run-id=$i &> $log_path/refuelB-12-50-conf-pred-$i.log
