...
parser.add_argument("folder", type=str, help="stats from which folder to use")
...
method_based_online_stats = {
    "unf": [
        ...
        ("UnfTime", "unf_time"),
        ("TotalTime", "totaltime"),
        ("MdpStates", "mdp_states"),
    ],
    "ff": [
        ("TrackTime", "track_time"),
        ("ReduceTime", "red_time"),
        ("TotalTime", "totaltime"),
        ("NrBeliefsBR", "origcard"),
        ("NrBeliefsAR", "redcard"),
        ("Dimension", "dim"),
    ],
}
...
def compute_online_statistics(folder, TPs, method):
    ...
    for tp in TPs:
        ...
        tp_statistics[tp] = {
            "timedOut": len(tables) - len(filtered_tables),
            "passed": len(filtered_tables),
        }

        if len(filtered_tables) > 0:
            tp_tables[tp] = pd.concat(
                [table[table["Index"] == tp - 1] for table in filtered_tables]
            )
            for stat in method_based_online_stats[method]:
                ...
                tp_statistics[tp][f"{id}_p10"] = tp_tables[tp][column_name].quantile(
                    0.1
                )
                tp_statistics[tp][f"{id}_avg"] = tp_tables[tp][column_name].mean()
                tp_statistics[tp][f"{id}_p90"] = tp_tables[tp][column_name].quantile(
                    0.9
                )
...
def compute_offline_statistics(folder, TPs, method):
    ...
    for tp in TPs:
        ...
        if len(filtered_tables) > 0:
            ...
            if method == "unf":
                tp_tables[tp]["UnfFrac"] = tp_tables[tp].apply(
                    lambda row: row.UnfTime / row.TotalTime, axis=1
                )
            if method == "ff":
                tp_tables[tp]["BElim"] = tp_tables[tp].apply(
                    lambda row: row.NrBeliefsBR - row.NrBeliefsAR, axis=1
                )
            # print(tp_tables[tp])
    ...
    with open("table1.tex", "w") as texfile:
        ...
        for idx, benchmark in enumerate(experiments):
            ...
            try:
                ...
                print(
                    f"Warning. (Likely: File {stats_folder}/{benchmark}-unf-ea/stats.out not found.)"
                )
            ...
            ffstats = compute_online_statistics(
                f"{stats_folder}/{benchmark}-ff-ch-ea", tracelengths[benchmark], "ff"
            )
            unfstats = compute_online_statistics(
                f"{stats_folder}/{benchmark}-unf-ea", tracelengths[benchmark], "unf"
            )
            texfile.write("} & ")
...
def export_totals_table():
    with open("table2.tex", "w") as texfile:
        ...
        for idx, benchmark in enumerate(experiments):
            ...
            texfile.write(str(idx + 1))
            texfile.write("\t& ")
            ff_nr_stats = compute_offline_statistics(
                f"{stats_folder}/{benchmark}-ff-nr-ea", tracelengths[benchmark], "ff"
            )
            ff_ch_stats = compute_offline_statistics(
                f"{stats_folder}/{benchmark}-ff-ch-ea", tracelengths[benchmark], "ff"
            )
            unf_ea_stats = compute_offline_statistics(
                f"{stats_folder}/{benchmark}-unf-ea", tracelengths[benchmark], "unf"
            )
            unf_fl_stats = compute_offline_statistics(
                f"{stats_folder}/{benchmark}-unf-fl", tracelengths[benchmark], "unf"
            )
            # texfile.write(" & ")
            for tl in tracelengths[benchmark]:
                if tl != tracelengths[benchmark][0]:
                ...
                if len(unf_ea_stats) > 0 and unf_ea_stats[tl]["passed"] > 0:
                    ...
                    texfile.write("\t& ")
                    texfile.write(
                        "{:.0f}".format(100 * unf_ea_stats[tl]["unffrac_avg"])
                    )
                    texfile.write("\t& ")
                    texfile.write(
                        "{:.0f}".format(100 * unf_ea_stats[tl]["unffrac_max"])
                    )
                    texfile.write("\t& ")
                else:
                    texfile.write("\t& ")
