...
def fn_fp_AUC_model_based(
    ...
    ax.plot(
        mean_ref_split_x,
        mean_fnr_rf_split_y,
        label="Refinement with Splitting FNR AUC",
        color="aqua",
        linestyle="--",
    )
    ax.plot(
        mean_ref_split_x,
        mean_fpr_rf_split_y,
        label="Refinement with Splitting FPR AUC",
        color="aqua",
        linestyle=":",
    )
...
def main_imc(args: argparse.Namespace):
    ...
    test_data["imc_transition_counts_ref_splitting"] = (
        imc_transition_counts_ref_splitting
    )
    ...
    if args.coarse:
        if args.high_st:
            file_name = os.path.join(
                args.out, f"testdata_rq_3_{args.mc}_coarse_high_st.pkl"
            )
        else:
