...
if __name__ == "__main__":
    ...
    for key, args in test_args.items():
        ...
        target_risks = []
        regression_risks = []
        conformal_prediction_risks = []
        conformal_prediction_rejections = []
        sampling_risks = []
        uncertain_risks = {}
        ...
        for trace in tqdm(traces):
            ...
            if state_estimator and error_estimator and cp_comb_class and rej_classifier and max_min: 
                ...
                model = mc_model(args.horizon)

                noisy_measurement = model.get_noisy_measurments(trace, args.horizon)

                new_noisy_scaled = -1+2*(noisy_measurement - max_min['dataset.MIN[1]'])/(max_min['dataset.MAX[1]']-max_min['dataset.MIN[1]'])
                ...
                conformal_prediction_risks.append(error_prob)
                conformal_prediction_rejections.append(keep_mask)
        ...
        if args.dump_stats is not None:
            ...
            if state_estimator and error_estimator and cp_comb_class and rej_classifier and max_min: 
                stats["risks"]["conformal_prediction_risks"] = conformal_prediction_risks
                stats["risks"]["conformal_prediction_rejections"] = conformal_prediction_rejections
