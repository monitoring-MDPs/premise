...
def fn_fp_comparison_model_based(
    ...
    for key in imc_risks_ref_splitting.keys():
        if key.split("-")[1] == str(max(imc_ref_splitting_ys)):
            imc_ref_splitting_final_risks[key.split("-")[0]] = imc_risks_ref_splitting[
                key
            ]
    ...
    for t in thresholds:
        ...
        for key in imc_ref_splitting_final_risks.keys():
            ...
            predictions_imc_ref_splitting = (
                np.array(imc_ref_splitting_final_risks[key], dtype=float) >= t
            )

            fp_imc_ref_splitting = np.sum(
                (predictions_imc_ref_splitting == 1) & (alarms == 0)
            )
            fn_imc_ref_splitting = np.sum(
                (predictions_imc_ref_splitting == 0) & (alarms == 1)
            )

            fnr_imc_ref_splitting = (
                fn_imc_ref_splitting / actual_positives if actual_positives > 0 else 0.0
            )
            fpr_imc_ref_splitting = (
                fp_imc_ref_splitting / actual_negatives if actual_negatives > 0 else 0.0
            )
    ...
    for key in imc_ref_splitting_final_risks.keys():
        ...
        imc_ref_splitting_fnr_aucs.append(
            np.trapz(imc_ref_splitting_fnr[key], thresholds)
        )
        imc_ref_splitting_fpr_aucs.append(
            np.trapz(imc_ref_splitting_fpr[key], thresholds)
        )
...
def fn_fp_AUC_model_based(
    ...
    for t in thresholds:
        ...
        for key in imc_risks_ref_splitting.keys():
            ...
            predictions_imc_ref_splitting = (
                np.array(imc_risks_ref_splitting[key], dtype=float) >= t
            )
    ...
    for x in imc_transition_counts.keys():
        imc_transition_counts_accumulated[x] = list(
            accumulate(imc_transition_counts[x])
        )

    for y in imc_transition_counts_ref.keys():
        imc_transition_counts_ref_accumulated[y] = list(
            accumulate(imc_transition_counts_ref[y])
        )

    for x in imc_transition_counts_ref_splitting.keys():
        imc_transition_counts_ref_splitting_accumulated[x] = list(
            accumulate(imc_transition_counts_ref_splitting[x])
        )
    ...
    for x in imc_transition_counts.keys():
        if (
            max(
                [
                    max(imc_transition_counts_accumulated[x]),
                    max(imc_transition_counts_ref_accumulated[x]),
                    max(imc_transition_counts_ref_splitting_accumulated[x]),
                ]
            )
            > y_range
        ):
    ...
    ax.plot(
        [0, int(y_range)],
        [auc_fnr_target, auc_fnr_target],
        label=f"Target FNR AUC (Final AUC: {float(auc_fnr_target):.3f})",
        color="black",
        linestyle="--",
    )
    ax.plot(
        [0, int(y_range)],
        [auc_fpr_target, auc_fpr_target],
        label=f"Target FPR AUC (Final AUC: {float(auc_fpr_target):.3f})",
        color="black",
        linestyle=":",
    )
    ...
    ax.plot(
        mean_no_ref_x,
        mean_fnr_no_rf_y,
        label=f"No refinement FNR AUC (Final AUC: {float(mean_fnr_no_rf_y[-1]):.3f})",
        color="red",
        linestyle="--",
        marker="P",
        markersize=4,
    ...
    ax.plot(
        mean_no_ref_x,
        mean_fpr_no_rf_y,
        label=f"No refinement FPR AUC (Final AUC: {float(mean_fpr_no_rf_y[-1]):.3f})",
        color="red",
        linestyle=":",
        marker="P",
        markersize=4,
    ...
    for key in imc_risks_ref_splitting.keys():
        imc_ref_splitting_fnr_aucs[key] = np.trapz(
            imc_ref_splitting_fnr[key], thresholds
        )
        imc_ref_splitting_fpr_aucs[key] = np.trapz(
            imc_ref_splitting_fpr[key], thresholds
        )
    ...
    for x in imc_ref_splitting_fnr_aucs.keys():
        for y in imc_ref_splitting_fnr_aucs_sorted.keys():
            if x.split("-")[0] == y:
                imc_ref_splitting_fnr_aucs_sorted[y].append(
                    [imc_ref_splitting_fnr_aucs[x]]
                )
    ...
    for x in imc_ref_splitting_fpr_aucs.keys():
        for y in imc_ref_splitting_fpr_aucs_sorted.keys():
            if x.split("-")[0] == y:
                imc_ref_splitting_fpr_aucs_sorted[y].append(
                    [imc_ref_splitting_fpr_aucs[x]]
                )
    ...
    fnr_rf_split_y_padded = [
        r + [np.nan] * (max_len - len(r)) for r in fnr_rf_split_y_flat
    ]
    ...
    fpr_rf_split_y_padded = [
        r + [np.nan] * (max_len - len(r)) for r in fpr_rf_split_y_flat
    ]
    ...
    ax.plot(
        mean_ref_split_x,
        mean_fnr_rf_split_y,
        label=f"Refinement with Splitting FNR AUC  (Final AUC: {float(mean_fnr_rf_split_y[-1]):.3f})",
        color="aqua",
        linestyle="--",
        marker="s",
        markersize=4,
    ...
    ax.plot(
        mean_ref_split_x,
        mean_fpr_rf_split_y,
        label=f"Refinement with Splitting FPR AUC   (Final AUC: {float(mean_fpr_rf_split_y[-1]):.3f})",
        color="aqua",
        linestyle=":",
        marker="s",
        markersize=4,
    ...
    for t in thresholds:
        ...
        for key in conformal_final_risks.keys():
            ...
            predictions_conformal = (
                np.array(conformal_final_risks[key], dtype=float) >= t
            )
            ...
            fnr_conformal = (
                fn_conformal / actual_positives if actual_positives > 0 else 0.0
            )
            fpr_conformal = (
                fp_conformal / actual_negatives if actual_negatives > 0 else 0.0
            )
