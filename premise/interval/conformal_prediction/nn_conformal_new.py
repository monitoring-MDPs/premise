...
parser.add_argument("--markov_chain", type=str, help="Markov Chain to learn")
...
print("Test empirical coverage on ESTIM STATES: ", ponsc_coverage, " (Expected = ", 1-opt.epsilon, ")")
...
for i in range(5): 
	...
	for x in range(opt.length - opt.horizon): 
		print(f'Step {x}')
...
print(f"Learned on {sample_count*(opt.length-1)} transitions")
...
with open(f"/workspaces/premise/premise/interval/conformal_prediction/test_results/{opt.markov_chain}_{opt.total_samples}_predictions", "wb") as fp:   
    pickle.dump(list(predictions), fp)

with open(f"/workspaces/premise/premise/interval/conformal_prediction/test_results/{opt.markov_chain}_{opt.total_samples}_alarms", "wb") as fp:   
