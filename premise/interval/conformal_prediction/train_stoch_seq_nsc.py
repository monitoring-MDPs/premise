...
class Train_StochSeqNSC:
    ...
    def train(self, n_epochs, batch_size, lr):
        ...
        if self.fine_tuning_flag:
            ...
            self.seq_nsc = torch.load(
                self.seq_nsc_path
                + "/seq_nsc_{}epochs.pt".format(self.seq_nsc_n_prev_epochs)
            )
            self.seq_nsc.eval()
            self.seq_se = torch.load(
                self.seq_se_path
                + "/seq_state_estimator_{}epochs.pt".format(self.seq_se_n_prev_epochs)
            )
            self.seq_se.eval()
        else:
            if self.net_type == "FF":
                n_hidden = 100
                self.seq_nsc = FF_SeqNSC(
                    input_size=int(self.seq_dataset.x_dim * self.seq_dataset.traj_len)
                )
                self.seq_se = FF_SeqSE(
                    int(self.seq_dataset.y_dim * self.seq_dataset.traj_len),
                    int(n_hidden),
                    int(self.seq_dataset.x_dim * self.seq_dataset.traj_len),
                )
            else:
                n_filters = 64
                self.seq_nsc = Conv_SeqNSC(
                    int(self.seq_dataset.x_dim), int(self.seq_dataset.traj_len)
                )
                self.seq_se = Conv_SeqSE(
                    int(self.seq_dataset.y_dim),
                    int(n_filters),
                    int(self.seq_dataset.x_dim),
                )
        ...
        optimizer_nsc = torch.optim.Adam(
            self.seq_nsc.parameters(), lr=lr
        )  # , betas=(opt.b1, opt.b2)
        optimizer_se = torch.optim.Adam(
            self.seq_se.parameters(), lr=lr
        )  # , betas=(opt.b1, opt.b2)

        self.seq_nsc_net_path = self.results_path + "/seq_nsc_{}epochs.pt".format(
            n_epochs
        )
        self.seq_se_net_path = self.results_path + "/seq_se_{}epochs.pt".format(
            n_epochs
        )
        ...
        for epoch in range(n_epochs):
            ...
            for i in range(bat_per_epo):
                ...
                comb_loss_fnc1 = w * nsc_loss_fnc(label_hypothesis1, T1t) + (
                    1 - w
                ) * se_loss_fnc(state_estim1, X1t)
                ...
                comb_loss_fnc2 = w * nsc_loss_fnc(label_hypothesis2, T2t) + (
                    1 - w
                ) * se_loss_fnc(state_estim2, X2t)
            ...
            val_comb_loss_fnc = w * nsc_loss_fnc(val_label_hypothesis, Tval_t) + (
                1 - w
            ) * se_loss_fnc(val_state_estim, Xval_t)
