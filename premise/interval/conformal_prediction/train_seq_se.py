...
class Train_SeqSE:
    ...
    def __init__(
        self, model_name, seq_dataset, net_type="FF", training_flag=True, idx=None
    ):
        ...
        if self.idx:
            self.results_path = (
                self.model_name + "/" + self.net_type + "_SeqSE_results/ID_" + self.idx
            )
    ...
    def train(self, n_epochs, batch_size, n_hidden=100, lr=0.0001):
        ...
        self.net_path = self.results_path + "/seq_state_estimator_{}epochs.pt".format(
            n_epochs
        )
        ...
        if self.net_type == "FF":
            ...
            self.seq_se = Conv_SeqSE(
                int(self.seq_dataset.y_dim), 128, int(self.seq_dataset.x_dim)
            )
        ...
        if self.training_flag:
            ...
            logger.info(
                f"Training {self.net_type} SeqSE for {n_epochs} epochs with {bat_per_epo} batches per epoch."
            )
            ...
            for epoch in range(n_epochs):
                ...
                for i in range(bat_per_epo):
                    ...
                    # Select a minibatch
                    if self.net_type == "FF":
                        X, Y, T = self.seq_dataset.generate_flat_mini_batches(
                            batch_size
                        )
                    else:
                ...
                if epoch % 50 == 0:
                    logger.info(f"Epoch= {epoch + 1},\t loss = {tmp_loss[-1]:2.4f}\t")
    ...
    def generate_test_results(self):
        ...
        if self.net_type == "FF":
            self.gen_estimates = np.empty(
                shape=(
                    self.seq_dataset.n_test_points,
                    self.seq_dataset.traj_len * self.seq_dataset.x_dim,
                )
            )
        else:
            self.gen_estimates = np.empty(
                shape=(
                    self.seq_dataset.n_test_points,
                    self.seq_dataset.x_dim,
                    self.seq_dataset.traj_len,
                )
            )

        for iii in range(self.seq_dataset.n_test_points):
            logger.info(f"Test point nb {iii + 1} / {self.seq_dataset.n_test_points}")
            if self.net_type == "FF":
                pred = self.seq_se(
                    Variable(Tensor([self.seq_dataset.Y_test_scaled_flat[iii]]))
                )
            else:
                pred = self.seq_se(
                    Variable(Tensor([self.seq_dataset.Y_test_scaled[iii].T]))
                )
            self.gen_estimates[iii] = pred.detach().cpu().numpy()[0]
        if self.net_type == "FF":
            self.gen_estimates_reshaped = np.reshape(
                self.gen_estimates,
                (
                    self.seq_dataset.n_test_points,
                    self.seq_dataset.traj_len,
    ...
    def plot_test_results(self):
        ...
        for i in range(self.seq_dataset.n_test_points):
            ...
            for j in range(self.seq_dataset.x_dim):
                axs[j].plot(
                    tspan, self.seq_dataset.X_test_scaled[i, :, j], color="blue"
                )
