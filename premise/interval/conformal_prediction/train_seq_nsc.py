...
class Train_SeqNSC:
    def __init__(
        ...
        if self.idx:
            self.results_path = (
                self.model_name + "/" + self.net_type + "_SeqNSC_resuts/ID_" + self.idx
            )
    ...
    def train(self, n_epochs, batch_size, lr):
        ...
        self.results_path = (
            self.model_name + "/" + self.net_type + "_SeqNSC_results/ID_" + self.idx
        )
        ...
        if self.net_type == "FF":
            ...
            self.seq_nsc = Conv_SeqNSC(
                x_dim=int(self.seq_dataset.x_dim),
                traj_len=int(self.seq_dataset.traj_len),
        ...
        logger.info(
            f"Training {self.net_type} SeqNSC for {n_epochs} epochs with {bat_per_epo} batches per epoch."
        )
