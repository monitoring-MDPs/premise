...
parser.add_argument("-s", "--samples", type=int, help="Total number of samples used in learning")
parser.add_argument("-l", "--sample_length", type=int, help="Path lenth, with horizon")
parser.add_argument("-ho", "--horizon", type=int, help="Horizon length")
parser.add_argument("--no-target", action="store_true", help="Do not use the target monitor" )
...
for x in range(round((args.samples - 50)*20/41)):
		path = suo.generate_random_traces([], args.sample_length)[0]
		trainset.append(tuple(path))
        
calibrset = []
for x in range(round((args.samples - 50)*6/41)):
		path = tuple(suo.generate_random_traces([], args.sample_length)[0])
...
for x in range(100):
		path = tuple(suo.generate_random_traces([], args.sample_length)[0])
		evaluation_samples.append(path)
...
#samples = np.load(f'/workspaces/premise/premise/interval/conformal_prediction/Datasets/{opt.markov_chain}/samples.npy', allow_pickle=True)
#ref_samples = np.load(f'/workspaces/premise/premise/interval/conformal_prediction/Datasets/{opt.markov_chain}/ref_samples.npy', allow_pickle=True)
#active_samples = np.load(f'/workspaces/premise/premise/interval/conformal_prediction/Datasets/{opt.markov_chain}/active_samples.npy', allow_pickle=True)
#evaluation_samples = np.load(f'/workspaces/premise/premise/interval/conformal_prediction/Datasets/{opt.markov_chain}/evaluation_samples.npy', allow_pickle=True)
...
nsc = Train_SeqNSC(model_name, dataset, net_type = args.net_type, nb_filters = args.nb_filters)
start_time = time.time()
...
comb_ponsc = Train_StochSeqNSC(model_name, dataset, net_type = args.net_type, fine_tuning_flag = args.do_finetuning, seq_nsc_idx = nsc_info, seq_se_idx = se_info)
start_time = time.time()
...
print("Box-Coverage for significance = ", 1-args.epsilon, ": ", se_box_coverage, "; Box Efficiency = ", se_box_efficiency)
...
print("Regr (NON-BOX) Coverage for significance = ", 1-args.epsilon, ": ", se_coverage, "; Efficiency = ", se_efficiency)

print("----- Computing test CP classification validity...")
print("Coverage on the test set states:")
nsc_coverage = cp_class.compute_coverage(eps=args.epsilon, inputs=state_test, outputs=output_test)
...
ponsc_coverage = cp_class.compute_coverage(eps=args.epsilon, inputs=estim_state_test, outputs=output_test)
print("Test empirical coverage on ESTIM STATES: ", ponsc_coverage, " (Expected = ", 1-args.epsilon, ")")

print("----- Computing test CP COMB classification validity...")
print("Coverage on the test set measurments:")
ponsc_coverage = cp_comb_class.compute_coverage(eps=args.epsilon, inputs=meas_test, outputs=output_test)
...
if args.do_refinement:
	...
	unc_meas_ref, unc_states_ref, unc_outputs_ref = utils.Comb_PONSC_active_sample_query(suo, ref_samples, args.sample_length, horizon, model_class=model, conf_pred = cp_comb_class, trained_svc = query_fnc, se_fnc = se_fnc, dataset=dataset)
...
for k in range(args.nb_active_iterations):
	...
	unc_meas, unc_states, unc_outputs = utils.Comb_PONSC_active_sample_query(suo, active_samples, args.sample_length, horizon, model_class=model, conf_pred = cp_comb_class, trained_svc = query_fnc, se_fnc = se_fnc, dataset=dataset)
	...
	active_comb_ponsc = Train_StochSeqNSC(model_name, active_dataset, net_type = args.net_type, fine_tuning_flag = args.do_finetuning, seq_nsc_idx = active_nsc_info, seq_se_idx = active_se_info)
	active_comb_ponsc.train(args.nb_epochs_active_tuning, args.batch_size, args.lr_tuning)
...
def infer_on_new_batch(new_noisy): 
	...
	return state_estim, pred_label, label_prob, error_prob, keep_mask

states = model.gen_trajectories(evaluation_samples, horizon)
...
state_estim, pred_label, label_prob, error_prob, keep_mask = infer_on_new_batch(noisy_measurements)

""" for i in range(5): 
	print('----------------------------------------------')
	print('Predicted states vs True states')
	for x in range(args.sample_length - args.horizon): 
		print(f'Step {x}')
...
print(f"Learned on {sample_count} samples")
print(f"Learned on {sample_count*(args.sample_length)} transitions")
...
for x in labels: 
	alarms.append(float(x))

print(alarms[:10])


#with open(f"/workspaces/premise/premise/interval/conformal_prediction/test_results/{args.markov_chain}_{args.total_samples}_predictions", "wb") as fp:   
 #   pickle.dump(list(predictions), fp)

#with open(f"/workspaces/premise/premise/interval/conformal_prediction/test_results/{args.markov_chain}_{args.total_samples}_alarms", "wb") as fp:   
 #   pickle.dump(list(alarms), fp)