...
def plot_mult_roc_curve(alarms_dict, risks_dict, title, fname=None):
    plt.figure(figsize=(8, 6))
    line_styles = ["-"]
    style_index = 0
    # plt.title("Receiver Operating Characteristic")
...
def plot_risk_histogram(risks, alarms, title, ax=None, fname=None):
    if ax is None:
        fig, ax = plt.subplots()
...
def main(stats_path="../../out/tmp/test4"):
    ...
    for stat_path in paths:
        data = np.load(stat_path, allow_pickle=True)
        ...
        plot_mult_roc_curve(alarm_dict, risks_dict, f"ROC curve {stat_path.name}")

        num_cols = 2
        num_rows = int(np.ceil(len(alarm_dict.keys()) / num_cols))
